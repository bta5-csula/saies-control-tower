...
- **Carbon Impact** — CO2e emissions breakdown by type, scope, carrier, and time period
- **Data Upload** — Upload your own Sales, Prices, and Carbon Emissions files; preview the cleaned and matched data. The app auto-detects the data format and currency.
...
The app auto-detects the uploaded file format. Two schemas are supported out of the box:

| File | ERPSim format | LAX / general format |
|---|---|---|
| Sales | MATERIAL_NUMBER, SIM_CALENDAR_DATE, QUANTITY, NET_VALUE, COST | Date, Material/Product, Quantity, Revenue, Cost |
| Prices | MATERIAL_NUMBER, SIM_CALENDAR_DATE, PRICE | Date, Material/Product, Price_USD_per_kg, Distribution_Channel |
| Carbon (optional) | TYPE, SUBTYPE, SCOPE, TOTAL_CO2E_EMISSIONS | Scope_1/2/3_CO2e_kg columns per row |

The app ships with default sample data. Use the Data Upload page to replace it with your own files for the current session. Additional sample datasets are in `sample_data/` for testing.
